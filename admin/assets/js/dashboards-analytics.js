...
if (typeof weeklyEarningReportsEl !== undefined && weeklyEarningReportsEl !== null) {
  fetch('admin/assets/php/dashboard-earnings.php')
  .then(response => response.json())
  .then(data => {
    console.log('Fetched Data:', data);

    const months = data.map(item => {
      const monthName = new Date(new Date().getFullYear(), item.month - 1, 1).toLocaleString('default', { month: 'short' });
      return monthName;
    });

    const activeMonthColor = '#bcd700'; // Change this to your desired color for the active month
      const defaultColor = '#e0e0e0';

    weeklyEarningReportsConfig.xaxis.categories = months;
    weeklyEarningReportsConfig.series[0].data = data.map(item => item.value);
    // Customize the tooltip label
    weeklyEarningReportsConfig.tooltip = {
      y: {
        formatter: function (value) {
          return 'Total Quote: ' + value;
        }
      },
      seriesLabel: false
    };

    weeklyEarningReportsConfig.colors = [
      '#bcd700','#e0e0e0',
      '#e0e0e0','#e0e0e0',
      '#e0e0e0','#e0e0e0',
      '#e0e0e0','#e0e0e0',
