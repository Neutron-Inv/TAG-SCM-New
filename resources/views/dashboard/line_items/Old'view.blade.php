                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($line_items as $items)
                                            <tr>
                                                ...
                                                @endphp
                                                <td style="text-align: right"> {{number_format($items->unit_cost) ?? 0}} </td>
                                                <td style="text-align: right"> {{number_format($items->unit_cost * $items->quantity ,2) ?? ''}} </td>

                                                <td style="text-align: right"> {{number_format($unitMargin, 2) ?? 0}} </td>

                                                <td style="text-align: right"> {{number_format($unitMargin * $items->quantity, 2) ?? ''}} </td>
                                                <td style="text-align: right"> {{number_format($unit_cost + $unitMargin, 2) ?? 0}} </td>
                                                <td style="text-align: right"> {{number_format($items->total_quote, 2) ?? ''}} </td>
                                                <td style="text-align: right"> {{$items->mesc_code }} </td>
                                                <td>
                                                    <a href="{{ route('line.duplicate',$items->line_id) }}" title="Duplicate Line Items" class="" onclick="return(duplicate());">
                                                        Duplicate
                                                    </a>
                                                </td>@php $tot = $items->total_quote;
