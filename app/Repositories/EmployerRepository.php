...
class EmployerRepository implements EmployerRepositoryInterface
{
    ...
    public function update(array $data, $employer_id)
    {
        $update = $this->model->findOrFail($employer_id);
        return $update->update($data);
    }
    // remove record from the database
    public function delete($employer_id)
    {
        return $this->model->destroy($employer_id);
    }
    // show the record with the givenbalance_id
    public function show($employer_id)
    {
        return $this->model->findOrFail($employer_id);
    }
